    ...
    <questions :questionTypeObj="questionTypeObj"
        ...
        <template v-if="questionData instanceof Object">
            ...
            <div slot="footer" class="footerBox">
                <el-button
                        v-if="questionAnswerArr.length >= allQuestionCount - 1 && !~questionAnswerArr.indexOf(selectQuestionObj.questionsId)"
                        :loading="loading" class="active"
                        @click="submit">
                    提交
                </el-button>
                <template v-else>
    ...
    methods: {
      ...
      init () {
        ...
        if (questionConf) {
          ...
          // 记录原来的试题数据
          this.getQuestionSeqCall(questionConf.questions);
      ...
      getQuestionSeqCall (arr) {
        ...
        arr.map(item => {
          item.questions.map((qItem, index) => {
            ...
            questionAnswerArr.includes(qItem.questionsId) && (qItem.answerResult = -1);
      ...
      add () {
        let opt = {
          ajaxSuccess: res => {
            this.getQuestionSeqCall(res.data.questions);
