        ...
        <div class="answer-content">
            ...
            <el-form ref="formValidate" :model="formValidate" label-width="100px">
                <el-form-item label="内容：" prop="content">
                    ...
                    <p class="tip">提示：描述此题的难点，自己做不出来的地方（不超过250字）。（{{content.length}}）字</p>
                </el-form-item>
                <el-form-item label="解题建议：" prop="result">
                    <simditor @valuechanged="getResult" :toolbarHidden="true"></simditor>
                    <p class="tip">提示：描述此题的难点，自己做不出来的地方（不超过250字）。{{result.length}}字</p>
                </el-form-item>
            </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      getResult (content) {//获取难题原因
        this.result = content.replace(/<\/?[^>]*>/g, '').replace(/[ | ]*\n/g, '\n').replace(/\n[\s| | ]*\r/g, '\n').replace(/&nbsp;/ig, '')
      ...
      * */
      listenSubEvent (isLoadingFun) {
        ...
        if (isSubmit) {
          ...
          isLoadingFun(true)
          if (this.result.length > 250 || this.content.length > 250) {
            this.errorMess('长度不大于250个字符')
            return false
          }
